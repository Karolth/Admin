        ...
        if (move_uploaded_file($_FILES["imagen"]["tmp_name"], $target_file)) {
            try {
                ...
                if ($e->getCode() == 23000) {
                    ...
                    echo json_encode(['success' => false, 'message' => ' Producto registrado. ' . $e->getMessage()]);
                }
            }
        }
    }
...
if ($input['action'] == "obtenerProducto") {
    $id = $input['id'];
...
if ($input['action'] == "editarProducto") {
  
            // Manejo de FormData para editar producto
            $id = $input['id'];
            $nombre = $input['nombre'];
            ...
            // Manejo de la imagen (si se envió)
            if (isset($_FILES['imagen']) && $_FILES['imagen']['error'] === UPLOAD_ERR_OK) {
                $imagen_nombre = $_FILES['imagen']['name'];
                $imagen_tmp = $_FILES['imagen']['tmp_name'];
                $imagen_ruta = "ruta/a/tus/imagenes/" . $imagen_nombre; // Ajusta la ruta

                // Mueve el archivo cargado a la ubicación deseada
                move_uploaded_file($imagen_tmp, $imagen_ruta);
            } else {
                $imagen_ruta = null; // O deja la ruta existente si no se cargó una nueva imagen
            }

            if (!empty($id) && !empty($nombre) && !empty($descripcion) && !empty($precio) && !empty($categoria) && !empty($proveedor)) {
                // Actualiza la base de datos, incluyendo la imagen si se cargó
                if ($imagen_ruta) {
                    $stmt = $pdo->prepare("UPDATE producto SET nombre_producto = ?, descripcion_producto = ?, precio_producto = ?, id_categoria = ?, id_proveedor = ?, imagen_producto = ? WHERE id_producto = ?");
                    $resultado = $stmt->execute([$nombre, $descripcion, $precio, $categoria, $proveedor, $imagen_ruta, $id]);
                } else {
                    ...
                    $resultado = $stmt->execute([$nombre, $descripcion, $precio, $categoria, $proveedor, $id]);
                }
